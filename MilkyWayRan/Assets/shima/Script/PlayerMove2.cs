...
public class PlayerMove2 : MonoBehaviour
{
    ...
    public AudioClip Player2_SE1;
    public AudioClip Player2_SE2;
    public AudioClip Player2_SE3;
    public AudioClip Player2_SE4;
    ...
    public void HP(int Number, int amount)
    {
        ...
        if (isCollision)
        {
            ...
            {
                //死んだ時の音
                audioSource.PlayOneShot(Player2_SE4);
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        if (TriggerFlag)
        {
            ...
            if (col.gameObject.name == EnemyName3)
            {
                ...
                //////
                var hit = col.gameObject;
                var health = hit.GetComponent<PlayerMove4>();
                if (health != null)
                {
                    //ここがプレイヤーの入力方向と逆の方向に無理やり飛ばす処理
                    _rigidBody.AddForce(-direction * 100, ForceMode.Impulse);
        ...
        {
            //小さい爆発の生成
            audioSource.PlayOneShot(Player2_SE2);
        ...
        {
            //小さい爆発の生成
            audioSource.PlayOneShot(Player2_SE3);
            var t = Instantiate(efect2, col.transform.position, Quaternion.identity);
            Debug.Log(PlayerNum + "がダメージをうけた");
            animator.SetBool("is_damage", true);
            Invoke("AnimatorBoolis_damage", 0.5f);
